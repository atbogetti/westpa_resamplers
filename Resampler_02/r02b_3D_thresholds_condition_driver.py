            ...
            if len(bin) == 0:
                ...
                if np.any(init_check):
                    ...
                    for idx in to_split_list:
                        if scaled_progresses[idx] != 0 and pcoords_d1[:,0][idx] < target_d1 and pcoords_d2[:,0][idx] > target_d2 and pcoords_d3[:,0][idx] < target_d3 and weights[idx]/split_into >= smallest_abs_weight:
                    ...
                    while np.sum(weights[start_merge_idx:start_merge_idx+6]) > largest_abs_weight:
                        start_merge_idx += 1

                    if start_merge_idx + 6 + 1 > (nsegs/2):
                        start_merge_idx = (nsegs/2) - 6 - 1
                    
                    to_merge_idx = to_merge_idx[start_merge_idx:start_merge_idx+6]
                    ...
                    print("merge weight check:", np.sum(weights[to_merge_idx]))
